/* 
	...
	Loads various shaders used by the project
*/
#include "shader.h"
#include <assert.h>
...
void Shader::Setup(const float time, const GLint * size, const GLfloat * projection, const GLfloat * modelview, const GLfloat * mvp, const GLfloat * nm)
{
...
bool Shader::Initialize(char * vertex_shader_file, char * fragment_shader_file)
{
	...
	this->texture_handle = glGetUniformLocation(this->program_id, (const GLchar *) "texture");
...
void CubemapShader::CommonSetup(const float time, const GLint * size, const GLfloat * projection, const GLfloat * modelview, const GLfloat * mvp, const GLfloat * nm, GLint texture_unit)
{
	if (this->time_handle != BAD_GL_VALUE)
		glUniform1f(this->time_handle, time);
	this->GLReturnedError("Shader::CommonSetup - after time_handle");
	if (this->size_handle != BAD_GL_VALUE && size != NULL)
		glUniform2iv(this->size_handle, 1, size);
	this->GLReturnedError("Shader::CommonSetup - after size_handle");
	if (this->projection_matrix_handle != BAD_GL_VALUE && projection != NULL)
		glUniformMatrix4fv(this->projection_matrix_handle, 1, GL_FALSE, projection);
	this->GLReturnedError("Shader::CommonSetup - after projection_matrix_handle");
	if (this->modelview_matrix_handle != BAD_GL_VALUE && modelview != NULL)
		glUniformMatrix4fv(this->modelview_matrix_handle, 1, GL_FALSE, modelview);
	this->GLReturnedError("Shader::CommonSetup - after modelview_matrix_handle");
	if (this->mvp_handle != BAD_GL_VALUE && mvp != NULL)
		glUniformMatrix4fv(this->mvp_handle, 1, GL_FALSE, mvp);
	this->GLReturnedError("Shader::CommonSetup - after mvp_handle");
	if (this->normal_matrix_handle != BAD_GL_VALUE && nm != NULL)
		glUniformMatrix3fv(this->normal_matrix_handle, 1, GL_FALSE, nm);
	this->GLReturnedError("Shader::CommonSetup - after normal_matrix_handle");
	if (this->cubemap_handle != BAD_GL_VALUE)
		glUniform1i(this->cubemap_handle, texture_unit);
	this->GLReturnedError("Shader::CommonSetup - after cubemap_handle");
}

void CubemapShader::Use()
{
	assert(this->program_id != BAD_GL_VALUE);
	glUseProgram(this->program_id);
}
...
bool CubemapShader::Initialize(char * vertex_shader_file, char * fragment_shader_file)
{
	GLint check_value;
	...
	// These uniforms are generic. They may or may not be present in an actual shader used by this program.
	...
	this->size_handle = glGetUniformLocation(this->program_id, (const GLchar *) "size");
	this->time_handle = glGetUniformLocation(this->program_id, (const GLchar *) "time");
	this->cubemap_handle = glGetUniformLocation(this->program_id, (const GLchar *) "cubemap");
	glUseProgram(0);

	return !GLReturnedError("Shader::Initialize - on exit");
}

void CubemapShader::CustomSetup()
{
}

void CubemapShader::TakeDown()
{
	GLint temp;
	GLsizei size;
...
bool CubemapShader::LoadShader(const char * file_name, GLuint shader_id)
{
	assert(file_name != NULL);
	if (GLReturnedError("Shader::LoadShader - on entrance"))
		return false;
...
stringstream CubemapShader::GetShaderLog(GLuint shader_id)
{
	stringstream s;
	GLint log_length;
	glGetShaderiv(shader_id, GL_INFO_LOG_LENGTH, &log_length);
...
bool CubemapShader::GLReturnedError(char * s)
{
	bool return_error = false;
	GLenum glerror;
...
LineShader::LineShader() : super()
{
	this->color_array_handle = BAD_GL_VALUE;
}

bool LineShader::Initialize(char * vertex_shader_file, char * fragment_shader_file)
{
...
void LineShader::CustomSetup(vec4 * color_array)
{
	glUniform4fv(this->color_array_handle, 10, (GLfloat *) color_array);
}
